...
# Activate virtual environment (platform-specific; see your OS docs)
# Example for macOS/Linux: source venv/bin/activate
...
#### 2. **GPU not detected or CUDA issues**
- Verify CUDA drivers and device availability on your machine.
- If you don't have GPU support, run the model on CPU by configuring your environment or framework appropriately (see Ultralytics docs for CPU-only usage).

#### 3. **Frontend can't reach backend**
- Ensure backend is running on `http://localhost:8000`
- Check CORS settings in `api.py`
- Verify firewall allows connections on the chosen ports

#### 4. **Image upload fails: Permission denied**
- Ensure the application process can write to `uploads/`, `outputs/`, and `logs/`. Adjust directory permissions or run the service with an appropriate user that has write access.

#### 5. **Model weights not found: FileNotFoundError**
```bash
# Download model weights (Ultralytics helper)
...
```markdown name=README_updated_cleaned.md url=https://github.com/harshitaukv/SafeSight-An-Intelligent-Workplace-Safety-Monitoring-and-PPE-Compliance-Detection-System/blob/main/README.md
# 🦺 SafeSight: AI-Based PPE Compliance & Workplace Safety Monitoring
...
> The architecture flowchart above replaces the older ASCII diagram with a mermaid flowchart for clearer visualization.
...
(Other sections such as API Reference, Training, Troubleshooting, Contributing, Roadmap, License, Author, and References remain; platform-specific or risky commands/snippets were removed or replaced with safe guidance.)
...
Next steps
- I can commit either of these cleaned versions back to the repository (update main or create a PR). Which would you like me to commit, if any?
- I can also: add a short note near the top explaining which troubleshooting commands were intentionally removed for safety, convert the mermaid diagrams to static SVGs, or create a PR so you can review changes before merging. Which would you prefer?
